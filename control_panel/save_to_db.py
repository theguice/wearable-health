...
def lumo_to_db(user_id, date_epoch, date_human, l_pct, l_act, l_delta):
    cur = conn.cursor()
    cur.execute("SELECT u_id,date_epoch FROM `wh_d_lumo` WHERE `u_id`=" + str(user_id) + " AND `date_epoch`=" + str(date_epoch) + " AND `act`='" + str(l_act) + "'")
    duplicate_check = cur.fetchone()

    if not duplicate_check:
        sql = "INSERT INTO `wh_d_lumo` (`u_id`,`date_epoch`,`date_human`,`pct`,`act`,`delta`) VALUES (" + str(user_id) + ",'" + str(date_epoch) + "','" + str(date_human) + "'," + str(l_pct) + ",'" + str(l_act) + "'," + str(l_delta) + ")";
        try:
            x.execute(sql)
            conn.commit()
        except:
        ...
        if (m["segments"]):
            for s in m["segments"]:
                for a in s["activities"]:
                    ...
                    ##### WRITE TO DB #####
                    cur = conn.cursor()
                    cur.execute("SELECT u_id,time_start,time_end,act FROM `wh_d_moves_acts` WHERE `u_id`=" + str(user_id) + " AND `time_start`='" + str(time_start) + "' AND `time_end`='" + str(time_end) + "' AND `act`='" + str(a['activity']) + "'")
                    duplicate_check = cur.fetchone()
                    if not duplicate_check:
                        sql = "INSERT INTO `wh_d_moves_acts` (`u_id`,`time_start`,`time_end`,`type`,`act`,`duration`, `distance`, `steps`) VALUES (" + str(user_id) + ",'" + str(time_start) + "','" + str(time_end) + "','" + str(s['type']) + "','" + str(a['activity']) + "','" + str(a['duration']) + "','" + str(a['distance'])  + "','" + str(steps) + "')";
                        try:
                            x.execute(sql)
                            conn.commit()
                        except:
    ...
    for m in moves:
        if (m["segments"]):
            ...
            for s in m["segments"]:
                ...
                ##### WRITE TO DB #####
                cur = conn.cursor()
                cur.execute("SELECT u_id FROM `wh_d_moves_places` WHERE `u_id`=" + str(user_id) + " AND `time_start`='" + str(time_start) + "' AND `time_end`='" + str(time_end) + "' AND `place_id`='" + str(s['place']['id']) + "'")
                duplicate_check = cur.fetchone()
                if not duplicate_check:
                    sql = "INSERT INTO `wh_d_moves_places` (`u_id`,`time_start`,`time_end`,`type`,`place_id`,`name`,`lat`, `lon`) VALUES (" + str(user_id) + ",'" + str(time_start) + "','" + str(time_end) + "','" + str(s['type']) + "','" + str(s['place']['id']) + "','" + str(name) + "','" + str(s['place']['location']['lat']) + "','" + str( s['place']['location']['lon']) + "')";
                    try:
                        x.execute(sql)
                        conn.commit()
                    except:
                    ...
                    for a in s['activities']:
                        if a['trackPoints']:
                            for t in a['trackPoints']:                                
                                ...
                                ##### WRITE TO DB #####
                                cur = conn.cursor()
                                cur.execute("SELECT u_id FROM `wh_d_moves_trackpoints` WHERE `u_id`=" + str(user_id) + " AND `time`='" + str(time) + "' AND `lat`='" + str(t['lat']) + "' AND `lon`='" + str(t['lon']) + "'")
                                duplicate_check = cur.fetchone()
                                if not duplicate_check:
                                    sql = "INSERT INTO `wh_d_moves_trackpoints` (`u_id`,`time`,`lat`, `lon`) VALUES (" + str(user_id) + ",'" + str(time) + "','" + str(t['lat']) + "','" + str(t['lon']) + "')";
                                    try:
                                        x.execute(sql)
                                        conn.commit()
                                    except:
